import { DURATION, getBezierPoint, getInterpolatedPoint, type Point, STYLE } from './lib';
...
export class CanvasRenderer {
  ...
  constructor(
    ...
    this.width = staticCtx.canvas.clientWidth;
    this.height = staticCtx.canvas.clientHeight;
  ...
  public drawStaticLayer(points: Point[]): void {
    ...
    // ① 베지에 곡선 가이드
    ctx.beginPath(); // 경로 시작
    ctx.moveTo(points[0].x, points[0].y); // 시작점으로 이동
